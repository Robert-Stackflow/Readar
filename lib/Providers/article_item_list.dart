...
import '../Database/Dao/article_item_dao.dart';
...
class ArticleItemList {
  ...
  Future<void> init() async {
    ...
    var items = (await ArticleItemDao.instance.query(
        loadLimit: loadLimit,
  ...
  /// 测试某个文章条目是否符合过滤要求
  ///
  bool testItem(ArticleItem item) {
    if (fids.isNotEmpty && !fids.contains(item.feedUid)) return false;
    if (filterType == FilterType.unread && item.read) return false;
